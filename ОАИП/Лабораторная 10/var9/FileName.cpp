...
int countVertexes(Tree* t, int k);
...
void main() {
	...
	for (;;)
	{
		...
		cout << "8 - подсчет количества вершин k\n";
		cout << "9 - выход\n";
		cout << ": ";
		cin >> choice;
		cout << "\n";
		...
		case 8:
			...
			cin >> k;
			cout << "Найдено " << countVertexes(Root, k) << " вершин" << endl;
			break;
		case 9:  exit(0);
		}
	}
...
int countVertexes(Tree* t, int k) {
	...
	count += countVertexes(t->Left, k);
	count += countVertexes(t->Right, k);
	return count;
}
